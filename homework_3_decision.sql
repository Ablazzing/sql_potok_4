...
do $$
	...
	loop		
		execute 'create table ' || bid_row.t_name || ' (id serial primary key, client_name varchar(100), amount numeric(12,2));';
...
do $$
	...
	begin
		create table if not exists credit_percent (client_name varchar(100), amount numeric(12,2));

		insert into credit_percent (client_name, amount)
		select client_name, amount * base_rate / count_days
		from company_credit
		union all
		select client_name, amount * person_rate / count_days
		from person_credit;

		raise notice 'Общая сумма уплаченных процентов: %', (select sum(amount) from credit_percent);
	end;
$$;
...
create view company_bid as (select * from bid where is_company = true);
